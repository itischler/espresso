...
public:
  ...
  void construct_lattice_model(LBRelaxationRates relaxation_rates, double kT,
                               unsigned int seed) {
    m_lattice_model = std::make_shared<LatticeModel>(LatticeModel(
        m_last_applied_force_field_id, real_c(kT), relaxation_rates.omega_bulk,
        relaxation_rates.omega_even, relaxation_rates.omega_odd,
        relaxation_rates.omega_shear, 1, seed));
  };
