...
template <typename FloatType> struct FFT_heffte<FloatType>::heffte_container {
...
FloatType greens_function(int x, int y, int z, Utils::Vector<int, 3> &dim) {
  ...
  return FloatType(-0.5) /
         FloatType(std::cos(2. * std::numbers::pi * FloatType(x) /
                            FloatType(dim[0])) +
                   std::cos(2. * std::numbers::pi * FloatType(y) /
                            FloatType(dim[1])) +
                   std::cos(2. * std::numbers::pi * FloatType(z) /
                            FloatType(dim[2])) -
                   3.) /
...
template <typename FloatType> void FFT_heffte<FloatType>::solve() {
  auto grid_range = get_lattice().get_local_grid_range();
  auto dim = grid_range.second - grid_range.first;
...
template class FFT_heffte<float>;
template class FFT_heffte<double>;
