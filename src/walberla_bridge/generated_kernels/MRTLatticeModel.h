...
{
   template< typename FieldPtrOrIterator >
   static void get( Matrix3< real_t > & pressureTensor, const MRTLatticeModel & lm, const FieldPtrOrIterator & it )
   {
        const auto x = it.x();
        const auto y = it.y();
        const auto z = it.z();
   ...
   template< typename PdfField_T >
   static void get( Matrix3< real_t > & pressureTensor , const MRTLatticeModel & lm , const PdfField_T & pdf,
                    const cell_idx_t x, const cell_idx_t y, const cell_idx_t z )
   {
        const real_t & xyz0 = pdf(x,y,z,0);
        const real_t f_0 = pdf.getF( &xyz0, 0);
