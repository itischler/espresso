...
void init_lb_walberla_magic_number(double viscosity, double magic_number, double density, double agrid,
  ...
  try {
    lb_walberla_instance = new_lb_walberla(viscosity, magic_number, density, agrid, tau,
...
void init_lb_walberla_relaxation_rates(LBRelaxationRates relaxation_rates, double density, double agrid,
                      double tau, const Utils::Vector3d &box_dimensions,
                      const Utils::Vector3i &node_grid, double kT,
                      unsigned int seed) {
  // Exceptions need to be converted to runtime errors so they can be
  // handled from Python in a parallel simulation
  try {
    lb_walberla_instance = new_lb_walberla(relaxation_rates, density, agrid, tau,
                                           box_dimensions, node_grid, kT, seed);
    lb_walberla_params_instance = new LBWalberlaParams{agrid, tau};
  } catch (const std::exception &e) {
    runtimeErrorMsg() << "Error during Walberla initialization: " << e.what();
...
void mpi_init_lb_walberla(double viscosity, double magic_number, double density,
  ...
  Communication::mpiCallbacks().call_all(init_lb_walberla_magic_number, viscosity, magic_number,
                                         density, agrid, tau, box_geo.length(),
                                         node_grid, kT, seed);
  if (lb_walberla_instance) {
...
void mpi_init_lb_walberla(LBRelaxationRates relaxation_rates, double density, double agrid,
                          double tau, double kT, unsigned int seed){
  Communication::mpiCallbacks().call_all(init_lb_walberla_relaxation_rates, relaxation_rates,
                                         density, agrid, tau, box_geo.length(),
                                         node_grid, kT, seed);
  if (lb_walberla_instance) {
    lb_lbfluid_set_lattice_switch(ActiveLB::WALBERLA);
